...
Daily Matchday Live Update v1 adds a static live/post-match support layer for fixture scores/status, official player points, and player `matchStatus`. It is loaded from generated browser-ready files and does not use runtime browser fetches. The live layer is display/support data only: it does not rerun recommendations, projections, finance metrics, score predictions, PELE data, or Team Builder weights.
...
- `liveMatchdayStatusData.js` - static site live/post-match fixture score and status support data
- `livePlayerStatusData.js` - static site official player points and matchStatus support data
...
This writes lightweight source JSON and browser-ready `window.LIVE_*` files for fixture scores/status, player actual points, and player `matchStatus`. It does not rerun projections, recommendations, finance metrics, score predictions, PELE data, or Team Builder weights.
