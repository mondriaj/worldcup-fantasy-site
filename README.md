...
- Captain Change Advisor v0 is a manual switch check. It does not know the user's full squad, does not track live fantasy scores, and assumes the replacement entered by the user is in their squad and has not played yet.
- Substitution Advisor v0 is a manual one-bench-player check. It does not know the user's full squad, cannot verify played/unplayed status, and flags different-position moves for manual formation checks.
