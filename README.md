...
Daily Matchday Live Update v1 adds a static live/post-match support layer for final fixture scores/status, official player points from completed fixtures, and player `matchStatus`. It is loaded from generated browser-ready files and does not use runtime browser fetches. The live layer is display/support data only: it does not rerun recommendations, projections, finance metrics, score predictions, PELE data, or Team Builder weights.
...
- `liveMatchdayStatusData.js` - static site final fixture score and status support data
- `livePlayerStatusData.js` - static site official player points from completed fixtures and matchStatus support data
- `scripts/importLiveMatchdayStatus.mjs` - build-time importer for FIFA fantasy players, squads, and rounds live support fields
- `scripts/validateLiveFixtureMapping.mjs` - QA check that live final scores map to the same local fixture/team pair as Match Environment and the World Cup page
- `data/liveMatchdayStatus_v1.json` - source live fixture/round status data generated by the live importer
- `data/livePlayerStatus_v1.json` - source player points and matchStatus data generated by the live importer
- `data/liveMatchdayStatusReport_v1.md` - live import report with coverage, validation, and update recommendation
- `data/liveFixtureMappingQa_v1.json` - generated live fixture mapping QA result
- `data/liveFixtureMappingQaReport_v1.md` - human-readable live fixture mapping QA summary
- `data/liveMatchdayUpdateModel_v1.md` - plain-language note for live scores/player points as display/support data separate from projections
...
- Captain Change Advisor is a manual switch check. Without a built/imported Team Builder squad it does not know the user's full squad; with saved-squad mode it can show imported official points/status where available, but it still cannot verify played/unplayed legality.
- Substitution Advisor is a manual one-bench-player check. Without a built/imported Team Builder squad it does not know the user's full squad; with saved-squad mode it can show imported official points/status where available, but it still cannot verify played/unplayed legality and flags different-position moves for manual formation checks.
...
- Matchday Decision Center can display static imported player points and `matchStatus` where available, but it does not infer played/unplayed legality, official deadlines, or formation legality. It organizes manual checks from the saved squad.
- Saved Squad Decision Mode can fill advisor fields from a built/imported Team Builder squad, but it still cannot verify played/unplayed status or official-game legality.
...
- Saved Squad Matchday Timeline uses model matchday projection kickoff labels and may show imported player points/status where available. It does not infer official fantasy deadlines or same-day captain/substitution legality.
...
Daily live matchday status import:

```bash
node scripts/importLiveMatchdayStatus.mjs
```

This writes lightweight source JSON and browser-ready `window.LIVE_*` files for final fixture scores/status, completed-fixture player actual points, and player `matchStatus`. It does not rerun projections, recommendations, finance metrics, score predictions, PELE data, or Team Builder weights.

Live fixture mapping QA:

```bash
node scripts/validateLiveFixtureMapping.mjs
```

This verifies that shown final scores use the resolved local fixture key and matching home/away team pair, and that in-progress fixture scores and player round points remain hidden until final.
