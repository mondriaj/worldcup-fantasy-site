...
const VERSION = "team_builder_optimizer_md2_v4";
const MATCHDAY_ID = "md2";
const DATA_FILES = [
  "playersData.js",
  "fantasyRulesData.js",
...
function bestRecommendation(recommendations = [], preferredMode = "balanced") {
  const matchdayRank = { md2: 0, group_stage_full: 1 };
  const modeRank = {
    [preferredMode]: 0,
    balanced: 1,
    captain: 2,
...
function playerFromOfficialRecord(record, lookups) {
  ...
  const startProbability = number(role?.md2StartProb, number(projection.start_probability));
  const expectedMinutes = number(role?.md2ExpectedMinutes, number(projection.expected_minutes));
...
function markdownReport(report) {
  ...
  return `# Team Builder Optimizer MD2 v4 QA
...
Greedy baseline starter MD2 points: **${display(report.greedyBaseline.starterProjectedPoints)}**.
...
async function main() {
  ...
  const roleModel = readJson("data/playerRoleModel_md2_v2.json");
  ...
  const report = {
    schemaVersion: "team_builder_qa_md2_v4",
    ...
    modelInputs: {
      projections: "fantasyPoolMatchdayProjectionsData.js / data/fantasyPoolMatchdayProjections_md2_v4.json",
    ...
    summary: {
      ...
      md2ProjectionRows: projections.length,
  ...
  await writeFile(projectPath("data/teamBuilderQa_md2_v4.json"), `${JSON.stringify(report, null, 2)}\n`);
  await writeFile(projectPath("data/teamBuilderQaReport_md2_v4.md"), markdownReport(report));

  if (report.failures.length) {
    console.error(`Team Builder MD2 v4 QA failed: ${report.failures.map((failure) => failure.id).join(", ")}`);
    process.exitCode = 1;
    return;
  }

  console.log(`Team Builder MD2 v4 QA ${report.status}. Balanced starter MD2 points ${display(balancedStarterProjected)}; gap vs greedy ${display(report.greedyBaseline.balancedGap)}.`);
}
