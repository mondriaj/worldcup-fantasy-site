...
const ACTIVE_VERSION = "20260618-md2-active-path";
...
function verifyPublicScripts(report, indexHtml, scriptJs) {
  ...
  if (!isRequiredOrderValid) {
    addFailure(report, "required_scripts_order", "Required active browser scripts are not loaded in the MD2 active-path order.", {
...
function verifyMd2Consistency(report, active, activeIdentity, browserPlayers) {
  const playersDataUniverse = identityUniverse(browserPlayers);
  const md2Recommendations = active.recommendationRows.filter((row) => matchdayId(row) === "md2");
  const md2Projections = active.projectionRows.filter((row) => matchdayId(row) === "md2");
  const md2ProjectionResolvedKeys = new Set(md2Projections
    .map((row) => {
    ...
    .filter(Boolean));
  const recommendationActiveIdentityMissing = rowsMissingActiveIdentity(md2Recommendations, activeIdentity);
  const projectionActiveIdentityMissing = rowsMissingActiveIdentity(md2Projections, activeIdentity);
  const financeActiveIdentityMissing = rowsMissingActiveIdentity(active.financeRows, activeIdentity);
  const recommendationProjectionMissing = md2Recommendations.filter((row) => {
    const officialId = activeOfficialIdForRecord(row, activeIdentity);
    return !officialId || !md2ProjectionResolvedKeys.has(`${officialId}|${matchdayId(row)}`);
  });
  const recommendationPlayersDataMissing = md2Recommendations.filter((row) => !resolvesToUniverse(row, playersDataUniverse));
  const projectionPlayersDataMissing = md2Projections.filter((row) => !resolvesToUniverse(row, playersDataUniverse));
  const financePlayersDataMissing = active.financeRows.filter((row) => !resolvesToUniverse(row, playersDataUniverse));
  const md2ScoreRows = active.scoreRows.filter((row) => matchdayId(row) === "md2");
  ...
  const invalidNumbers = [
    ...validateFiniteNumbers(md2Recommendations, "md2_recommendation"),
    ...validateFiniteNumbers(md2Projections, "md2_projection"),
    ...validateFiniteNumbers(active.financeRows, "finance_metric"),
    ...validateFiniteNumbers(active.scoreRows, "score_fixture")
  ];

  if (!md2Recommendations.length) {
    addFailure(report, "md2_recommendations_missing", "No MD2 recommendation candidates are present.", {});
  }

  if (!md2Projections.length) {
    addFailure(report, "md2_projections_missing", "No MD2 matchday projection rows are present.", {});
  }

  if (recommendationActiveIdentityMissing.length) {
    addFailure(report, "md2_recommendations_missing_active_official_identity", "Some MD2 recommendation candidates do not resolve to the active official fantasy pool.", {
  ...
  if (recommendationProjectionMissing.length) {
    addFailure(report, "md2_recommendations_missing_projection", "Some MD2 recommendation candidates do not have matching MD2 projection rows after active official fantasy identity resolution.", {
  ...
  if (projectionActiveIdentityMissing.length) {
    addFailure(report, "md2_projections_missing_active_official_identity", "Some MD2 projection rows do not resolve to the active official fantasy pool.", {
  ...
  if (!md2ScoreRows.length) {
    addFailure(report, "md2_score_fixture_coverage_missing", "Score prediction rows do not cover MD2 fixtures.", {});
  }
  ...
  const consistency = {
    md2_recommendation_rows: md2Recommendations.length,
    md2_projection_rows: md2Projections.length,
    md2_score_fixture_rows: md2ScoreRows.length,
    md2_recommendations_resolving_active_identity_count: md2Recommendations.length - recommendationActiveIdentityMissing.length,
    md2_recommendations_missing_active_identity_count: recommendationActiveIdentityMissing.length,
    md2_projections_missing_active_identity_count: projectionActiveIdentityMissing.length,
    ...
    md2_recommendations_missing_players_data_enrichment_count: recommendationPlayersDataMissing.length,
    md2_projections_missing_players_data_enrichment_count: projectionPlayersDataMissing.length,
  ...
  addCheck(report, "md2_recommendation_identity_and_projection_coverage", recommendationActiveIdentityMissing.length || recommendationProjectionMissing.length ? "fail" : "pass", consistency);
  addCheck(report, "md2_score_fixture_coverage", md2ScoreRows.length && !duplicateScoreFixtureKeys.length ? "pass" : "fail", {
    md2_score_fixture_rows: md2ScoreRows.length,
    duplicate_score_fixture_key_count: duplicateScoreFixtureKeys.length
  });
...
function buildMarkdownReport(report) {
  ...
  const md2 = report.md2_consistency || {};
  const lines = [];
  lines.push("# Active MD2 Data Flow QA Report");
  lines.push("");
  ...
  lines.push("## MD2 Gate");
  lines.push("");
  lines.push(`- MD2 recommendations: ${md2.md2_recommendation_rows ?? "n/a"}`);
  lines.push(`- MD2 recommendation active identity coverage: ${md2.md2_recommendations_resolving_active_identity_count ?? "n/a"} / ${md2.md2_recommendation_rows ?? "n/a"}`);
  lines.push(`- MD2 recommendation projection misses: ${md2.recommendation_projection_missing_count ?? "n/a"}`);
  lines.push(`- MD2 projections: ${md2.md2_projection_rows ?? "n/a"}`);
  lines.push(`- MD2 score fixtures: ${md2.md2_score_fixture_rows ?? "n/a"}`);
  lines.push(`- playersData.js MD2 recommendation enrichment misses: ${md2.md2_recommendations_missing_players_data_enrichment_count ?? "n/a"}`);
  lines.push(`- playersData.js MD2 projection enrichment misses: ${md2.md2_projections_missing_players_data_enrichment_count ?? "n/a"}`);
  lines.push(`- playersData.js finance enrichment misses: ${md2.finance_missing_players_data_enrichment_count ?? "n/a"}`);
  lines.push("");
  lines.push("## Checks");
  lines.push("");
...
async function main() {
  const report = {
    schema_version: "active_md2_data_flow_qa_v1",
  ...
  report.md2_consistency = verifyMd2Consistency(report, active, activeIdentity, sync.browserPlayers);
  ...
  await writeFile(projectPath("data/activeMd2DataFlowQa.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
  await writeFile(projectPath("data/activeMd2DataFlowQaReport.md"), buildMarkdownReport(report), "utf8");

  console.log(`data/activeMd2DataFlowQa.json: ${report.status}`);
