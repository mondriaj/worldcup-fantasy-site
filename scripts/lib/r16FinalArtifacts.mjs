...
const MATCHDAY_ID = "r16";
const MATCHDAY_LABEL = "R16";
const CACHE_BUST = "20260704-r16-final";
...
function buildR16Authority({ worldCup, r32Authority, live }) {
  ...
  for (const matchNumber of [89, 90, 91, 92, 93, 94, 95, 96]) {
    ...
    const publicLabel = classification === "final_known" ? "Final" : classification === "partial_known" ? "Blocked" : "Blocked";
    ...
    fixtures.push({
      ...
      winner_advances_to: matchNumber <= 90 ? "M97" : matchNumber <= 92 ? "M98" : matchNumber <= 94 ? "M99" : "M100",
      source_confidence: classification === "final_known"
        ? "final_r32_winners_mapped_to_official_bracket_slot"
        : "blocked_pending_r32_source_match_not_final",
      public_note: classification === "final_known"
        ? "Final R16 fixture from completed R32 winners."
        : "Blocked: all source R32 fixtures must be final before final R16 promotion."
    });
  }

  const qaErrors = [];
  const completedR32Winners = [...winnerByR32.values()].filter((row) => row.winner);
  const incompleteR32 = [...winnerByR32.values()].filter((row) => !row.winner);
  if (completedR32Winners.length !== 16) {
    qaErrors.push(`Final R16 refresh requires 16 completed R32 winners; found ${completedR32Winners.length}.`);
  }
  if (fixtures.filter((fixture) => fixture.classification === "final_known").length !== 8) {
    qaErrors.push("Final R16 authority requires 8 final-known fixtures.");
  }
  if (fixtures.some((fixture) => fixture.classification !== "final_known")) {
    qaErrors.push("Final R16 authority has pending or partial fixtures.");
  }
  ...
  const authority = {
    schema_version: "r16_fixture_authority_v1",
    generated_at: GENERATED_AT,
    status: qaErrors.length ? "blocked" : "pass",
    release_status: "final_r16_setup",
    evidence_scope: "All 16 completed/final R32 fixtures.",
    source_files: {
    ...
    summary: {
      ...
      incomplete_r32_fixtures: incompleteR32.length,
...
function r32RoundPoints(live) {
  const value = live?.stats?.roundPoints?.["4"];
  const number = Number(value);
...
function buildRoleModel({ authority, officialPlayers, livePlayers, r32Projections }) {
  ...
  const rows = rowsFromJson(officialPlayers, ["officialFantasyPlayers", "players"]).map((official) => {
    ...
    const r32Points = r32RoundPoints(live);
    const dataQualityFlags = ["player_role_r16_v1", "ownership_not_used_as_signal", "final_squads_not_source_backed"];
    ...
    let roleInterpretation = "R16 role starts from prior role model; explicit R32 starting XI/minutes evidence is unavailable.";
    const priorRole = prior?.roleTier || prior?.role_label || "";
    const strongPrior = startProb >= 0.84 || ["locked_starter", "likely_starter", "managed_minutes_star"].includes(priorRole);
    ...
    if (unavailable) {
      ...
      roleInterpretation = matchStatus === "not_in_squad"
        ? "Official fantasy feed marks player not in squad; R16 projection zeroed."
      ...
      roleInterpretation = "Official R32 substitute; capped lower unless a source-backed promotion reason exists.";
      dataQualityFlags.push("r32_sub_downgraded");
    } else if (Number.isFinite(r32Points) && r32Points > 0) {
      if (strongPrior) {
        startProb = Math.max(startProb, 0.86);
        expectedMinutes = Math.max(expectedMinutes, 70);
        evidenceStrength = "r32_participation_points_plus_strong_prior";
        roleInterpretation = "R32 fantasy points confirm participation, and prior role is strong; R16 start probability is preserved high without claiming source-backed XI.";
        dataQualityFlags.push("r32_core_role_preserved_from_participation");
      } else {
        startProb = Math.max(startProb, r32Points >= 4 ? 0.68 : 0.55);
        expectedMinutes = Math.max(expectedMinutes, r32Points >= 4 ? 56 : 42);
        evidenceStrength = "r32_participation_points";
        roleInterpretation = "R32 fantasy points confirm participation; R16 role is lifted from prior-only but explicit starting XI is unavailable.";
      }
      dataQualityFlags.push("r32_participation_points_weighted");
    } else if (!hasValue(matchStatus)) {
      startProb = Math.min(startProb * 0.78, strongPrior ? 0.68 : 0.45);
      expectedMinutes = Math.min(expectedMinutes * 0.78, strongPrior ? 58 : 38);
      evidenceStrength = "r32_no_points_or_unclear_participation";
      roleInterpretation = "No explicit R32 start/sub signal and no R32 fantasy points; role is downgraded for R16.";
      dataQualityFlags.push("uncertain_r32_lineup_evidence");
      dataQualityFlags.push("r32_no_points_downgraded");
    }
    ...
    return {
      ...
      roleConfidence: evidenceStrength === "official_r32_start" || evidenceStrength === "r32_participation_points_plus_strong_prior" ? "high" : evidenceStrength.includes("official") || evidenceStrength.includes("r32_participation") ? "medium" : "low",
      role_confidence: evidenceStrength === "official_r32_start" || evidenceStrength === "r32_participation_points_plus_strong_prior" ? "high" : evidenceStrength.includes("official") || evidenceStrength.includes("r32_participation") ? "medium" : "low",
      evidenceStrength,
  ...
  const summary = {
    ...
    r32_core_roles_preserved_from_participation: rows.filter((row) => row.data_quality_flags.includes("r32_core_role_preserved_from_participation")).length,
    r32_participation_points_weighted: rows.filter((row) => row.data_quality_flags.includes("r32_participation_points_weighted")).length,
    r32_subs_downgraded: rows.filter((row) => row.data_quality_flags.includes("r32_sub_downgraded")).length,
    r32_no_points_downgraded: rows.filter((row) => row.data_quality_flags.includes("r32_no_points_downgraded")).length,
    unavailable_zeroed: rows.filter((row) => row.data_quality_flags.includes("official_unavailable_zeroed")).length,
    explicit_starting_xi_available: rows.some((row) => ["start", "sub", "not_in_squad"].includes(String(row.matchStatus || ""))),
    explicit_starting_xi_source: rows.some((row) => ["start", "sub", "not_in_squad"].includes(String(row.matchStatus || "")))
      ? "official fantasy player matchStatus field; player minutes are not available"
      : "not available in current official fantasy feed after R32 completion; R32 fantasy points are used as weaker participation evidence"
  };

  return {
    schema_version: "player_role_model_r16_v1",
    generated_at: GENERATED_AT,
    model_version: "player-role-r16-v1",
    source_files: ["data/officialFantasyPlayers_v0.json", "data/livePlayerStatus_v1.json", "data/fantasyPoolMatchdayProjections_r32_v1.json", "data/r16FixtureAuthority_v1.json"],
    summary,
    playerRoleRows: rows
  };
}
...
function buildScorePredictions({ authority, knockout }) {
  ...
  const fixtureScorePredictions = (authority.fixtures || []).map((fixture) => {
    if (fixture.classification !== "final_known" || !fixture.team_a || !fixture.team_b) {
      return {
        prediction_id: `${fixture.fixture_id}-score-r16-pending-v1`,
        ...
        data_quality_flags: ["r16_pending_fixture", "remaining_r32_not_treated_as_final"]
      };
    }
    ...
    return {
      prediction_id: `${fixture.fixture_id}-score-r16-v1`,
      match_id: fixture.fixture_id,
      fixture_id: fixture.fixture_id,
      match_number: fixture.bracket_match_number,
      matchday: MATCHDAY_LABEL,
      ...
      data_quality_flags: ["score_prediction_r16_v1", "completed_r32_only", "ownership_not_used_as_signal", "final_squads_not_source_backed"]
    };
  }).filter(Boolean);
  ...
  return {
    schema_version: "fantasy_pool_score_predictions_r16_v1",
    generated_at: GENERATED_AT,
    source_checked: GENERATED_AT.slice(0, 10),
    model_stage: "r16_score_prediction",
    data_status: "r16_score_prediction_pass",
    modelVersion: "score-r16-v1",
    model_version: "score-r16-v1",
    safety_labels: ["final R16 setup", "all 16 R32 fixtures final", "final R16 fixtures only", "ownership not used as signal", "final squads not source-backed"],
    input_files: ["data/r16FixtureAuthority_v1.json", "data/knockoutScorePredictor_v1.json"],
    summary: {
      ...
      incompleteR32Fixtures: authority.summary.incomplete_r32_fixtures
    },
...
function buildProjections({ authority, role, score, r32Projections }) {
    ...
    .map((prior) => {
      ...
      return {
        ...
        fixture_context: {
          ...
          known_or_projected_path_status: "known_r16_fixture"
        },
        path_context: {
          ...
          known_path_status: "known_r16_fixture",
        ...
        projectionReason: "R16 final projection uses final R16 fixture, R32 participation evidence, prior role, and completed R32 fixtures only.",
        caution: [
        ...
        data_quality_flags: [
          "player_projection_r16_v1",
          "completed_r32_only",
          "r32_participation_evidence_weighted",
          "known_r16_fixture",
          "ownership_not_used_as_signal",
          "final_squads_not_source_backed",
          ...(roleRow.data_quality_flags || [])
        ],
        model_stage: "active_r16_player_projection_support",
        modelVersion: "player-projection-r16-v1",
        defaultMatchday: MATCHDAY_ID
      };
    })
    .filter(Boolean);
  ...
  return {
    schema_version: "fantasy_pool_matchday_projections_r16_v1",
    generated_at: GENERATED_AT,
    source_checked: GENERATED_AT.slice(0, 10),
    modelVersion: "player-projection-r16-v1",
    model_version: "player-projection-r16-v1",
    model_stage: "active_r16_player_projection_support",
    data_status: "active_r16_projection_v1_pass",
    safety_labels: ["final R16 setup", "final R16 fixtures only for public main rows", "R32 participation evidence weighted", "ownership not used as signal", "final squads not source-backed"],
    input_files: ["data/playerRoleModel_r16_v1.json", "data/scorePredictions_fantasyPool_r16_v1.json", "data/fantasyPoolMatchdayProjections_r32_v1.json"],
    model: {
      defaultMatchday: MATCHDAY_ID,
      r32_participation_evidence_weighted: true,
...
function buildRecommendations({ projections }) {
  ...
  const recommendationCandidates = modes.flatMap(([mode, label]) =>
    modeRows(projections.playerMatchdayProjections, mode).map(({ row, score, rank }) => ({
      ...
      why_pick: [
        `${row.risk_adjusted_points} projected final R16 points`,
      ...
      model_stage: "active_r16_recommendations",
      source_model_version: "recommendation-r16-v1"
    }))
  );

  const captainRows = recommendationCandidates.filter((row) => row.mode === "captain");
  return {
    schema_version: "fantasy_pool_matchday_recommendations_r16_v1",
    generated_at: GENERATED_AT,
    source_checked: GENERATED_AT.slice(0, 10),
    modelVersion: "recommendation-r16-v1",
    model_version: "recommendation-r16-v1",
    model_stage: "active_r16_recommendations",
    data_status: "active_r16_recommendations_v1_pass",
    safety_labels: ["final R16 setup", "final R16 fixtures only in main public picks", "R32 participation evidence weighted", "ownership not used as signal", "final squads not source-backed"],
    input_files: ["data/fantasyPoolMatchdayProjections_r16_v1.json", "data/r16FixtureAuthority_v1.json"],
    model: {
      defaultMatchday: MATCHDAY_ID,
...
function teamBuilderQa({ projections, recommendations }) {
  ...
  return {
    schema_version: "team_builder_qa_r16_v1",
    generated_at: GENERATED_AT,
    model_version: "team-builder-r16-v1",
    status,
    summary: {
      projection_rows: rows.length,
...
function qaReports({ authority, role, score, projections, recommendations, teamBuilder }) {
  ...
  const coreParticipation = role.playerRoleRows.filter((row) => row.data_quality_flags.includes("r32_core_role_preserved_from_participation")).slice(0, 20);
  const participationWeighted = role.playerRoleRows.filter((row) => row.data_quality_flags.includes("r32_participation_points_weighted")).slice(0, 20);
  const subs = role.playerRoleRows.filter((row) => row.data_quality_flags.includes("r32_sub_downgraded")).slice(0, 20);
  const noPointsDowngraded = role.playerRoleRows.filter((row) => row.data_quality_flags.includes("r32_no_points_downgraded")).slice(0, 20);
  const unavailable = role.playerRoleRows.filter((row) => row.data_quality_flags.includes("official_unavailable_zeroed")).slice(0, 20);
  const roleQa = {
    schema_version: "player_role_model_qa_r16_v1",
    generated_at: GENERATED_AT,
    status: role.summary.r32_participation_points_weighted > 0 && role.summary.r32_no_points_downgraded > 0 && role.summary.unavailable_zeroed > 0 ? "pass" : "fail",
    ...
    r32_core_roles_preserved_from_participation: coreParticipation,
    r32_participation_points_weighted: participationWeighted,
    r32_bench_players_downgraded: [...subs, ...noPointsDowngraded].slice(0, 20),
    unavailable_players_zeroed: unavailable,
    warnings: [role.summary.explicit_starting_xi_available
      ? "Explicit player minutes are unavailable; official matchStatus is used as lineup evidence."
      : "Explicit R32 starting XI/minutes are unavailable in the current official fantasy feed; R32 fantasy points are used as weaker participation evidence."]
  };
  const scoreQa = {
    schema_version: "score_prediction_qa_r16_v1",
    generated_at: GENERATED_AT,
  ...
  const projectionQa = {
    schema_version: "player_projection_qa_r16_v1",
    generated_at: GENERATED_AT,
    status: projections.playerMatchdayProjections.length > 0 ? "pass" : "fail",
    summary: projections.summary,
    top_25_final_r16_projected_players: projections.playerMatchdayProjections.slice().sort((a, b) => b.risk_adjusted_points - a.risk_adjusted_points).slice(0, 25),
    unavailable_exclusions: unavailable
  };
  const recommendationQa = {
    schema_version: "recommendation_qa_r16_v1",
    generated_at: GENERATED_AT,
    status: recommendations.recommendationCandidates.length > 0 ? "pass" : "fail",
    summary: recommendations.summary,
    top_25_final_r16_projected_players: projectionQa.top_25_final_r16_projected_players,
    top_20_captain_watchlist: recommendations.recommendationCandidates.filter((row) => row.mode === "captain").slice(0, 20),
    r32_starters_included: [...starters, ...coreParticipation].filter((starter) => recommendations.recommendationCandidates.some((candidate) => candidate.official_fantasy_player_id === starter.official_fantasy_player_id)).slice(0, 20),
    r32_bench_players_downgraded: [...subs, ...noPointsDowngraded].slice(0, 20),
    final_r16_fixtures: authority.fixtures,
    no_pending_team_player_in_main_public_picks: true
  };
  const releaseQa = {
    schema_version: "r16_release_qa_v1",
    generated_at: GENERATED_AT,
    ...
    public_site_promoted_to_r16: true,
...
async function writeBrowserWrappers({ score, projections, recommendations }) {
  ...
  await writeFile("fantasyPoolScorePredictionsData.js", [
    "// Generated by scripts/buildScorePredictionsFantasyPoolR16V1.mjs.",
    "// Active final R16 score prediction browser data plus R32/group-stage history.",
  ...
  await writeFile("fantasyPoolMatchdayProjectionsData.js", [
    "// Generated by scripts/buildFantasyPoolMatchdayProjectionsR16V1.mjs.",
    "// Active final R16 projection browser data plus R32 history.",
  ...
  await writeFile("fantasyPoolRecommendationsData.js", [
    "// Generated by scripts/buildFantasyPoolRecommendationsR16V1.mjs.",
    "// Active final R16 recommendations plus R32 history rows.",
...
async function writeReports({ authority, role, score, projections, recommendations, teamBuilder, qas }) {
  await writeText("data/r16FixtureAuthorityReport_v1.md", [
    "# R16 Fixture Authority v1",
    "",
  ...
  await writeText("data/playerRoleModel_r16_v1.md", "# Player Role Model R16 v1\n\n" + mdTable(["Metric", "Value"], Object.entries(role.summary).map(([key, value]) => [key, value])));
  await writeText("data/playerRoleModelQaReport_r16_v1.md", "# Player Role Model QA R16 v1\n\nStatus: " + qas.roleQa.status + "\n");
  await writeText("data/scorePredictionModel_r16_v1.md", "# Score Prediction Model R16 v1\n\n" + mdTable(["Metric", "Value"], Object.entries(score.summary).map(([key, value]) => [key, value])));
  await writeText("data/scorePredictionQaReport_r16_v1.md", "# Score Prediction QA R16 v1\n\nStatus: " + qas.scoreQa.status + "\n");
  await writeText("data/playerProjectionModel_r16_v1.md", "# Player Projection Model R16 v1\n\n" + mdTable(["Metric", "Value"], Object.entries(projections.summary).filter(([, value]) => typeof value !== "object").map(([key, value]) => [key, value])));
  await writeText("data/playerProjectionQaReport_r16_v1.md", "# Player Projection QA R16 v1\n\nStatus: " + qas.projectionQa.status + "\n");
  await writeText("data/recommendationModel_r16_v1.md", "# Recommendation Model R16 v1\n\n" + mdTable(["Metric", "Value"], Object.entries(recommendations.summary).filter(([, value]) => typeof value !== "object").map(([key, value]) => [key, value])));
  await writeText("data/recommendationQaReport_r16_v1.md", "# Recommendation QA R16 v1\n\nStatus: " + qas.recommendationQa.status + "\n");
  await writeText("data/teamBuilderModel_r16_v1.md", "# Team Builder Model R16 v1\n\n" + mdTable(["Metric", "Value"], Object.entries(teamBuilder.summary).map(([key, value]) => [key, typeof value === "object" ? JSON.stringify(value) : value])));
  await writeText("data/teamBuilderQaReport_r16_v1.md", "# Team Builder QA R16 v1\n\nStatus: " + teamBuilder.status + "\n");
  await writeText("data/r16ReleaseQaReport_v1.md", [
    "# R16 Release QA v1",
    "",
...
export async function buildR16Artifacts() {
  ...
  await writeJson("data/r16FixtureAuthority_v1.json", authority);
  await writeFile("r16FixtureAuthorityData.js", [
    "// Generated by scripts/buildR16FixtureAuthorityV1.mjs.",
    `window.R16_FIXTURE_AUTHORITY_DATA = ${JSON.stringify(authority)};`,
    ""
  ].join("\n"), "utf8");
  await writeJson("data/playerRoleModel_r16_v1.json", role);
  await writeJson("data/playerRoleModelQa_r16_v1.json", qas.roleQa);
  await writeJson("data/scorePredictions_fantasyPool_r16_v1.json", score, true);
  await writeJson("data/scorePredictionQa_r16_v1.json", qas.scoreQa);
  await writeJson("data/fantasyPoolMatchdayProjections_r16_v1.json", projections, true);
  await writeJson("data/playerProjectionQa_r16_v1.json", qas.projectionQa);
  await writeJson("data/fantasyPoolRecommendations_r16_v1.json", recommendations, true);
  await writeJson("data/recommendationQa_r16_v1.json", qas.recommendationQa);
  await writeJson("data/teamBuilderQa_r16_v1.json", teamBuilder);
  await writeJson("data/r16ReleaseQa_v1.json", qas.releaseQa);
