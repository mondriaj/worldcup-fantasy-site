...
function teamVolatility(rows) {
  ...
  for (const [teamId, list] of byTeam.entries()) {
    const highPriorNoR16 = list.filter((row) => row.prior_start_probability >= 0.74 && !row.r16_participated).length;
    const r16WithoutR32 = list.filter((row) => row.r16_participated && !row.r32_participated).length;
    const repeatedParticipation = list.filter((row) => row.r16_participated && row.r32_participated).length;
    const eligible = Math.max(1, list.filter((row) => Number(row.start_probability) > 0).length);
    const score = clamp((highPriorNoR16 * 0.8 + r16WithoutR32 * 0.45) / eligible, 0, 0.42);
    out.set(teamId, {
      team_id: teamId,
      ...
      high_prior_without_r16_points: highPriorNoR16,
      r16_points_without_r32_points: r16WithoutR32,
      r32_r16_participation_continuity: repeatedParticipation
    });
  }
  return out;
}

function buildRoleModel({ authority, officialPlayers, livePlayers, r16Projections }) {
  ...
  const rows = rowsFromJson(officialPlayers, ["officialFantasyPlayers", "players"]).map((official) => {
    ...
    const dataQualityFlags = ["player_role_qf_v1", "ownership_not_used_as_signal", "final_squads_not_source_backed", "explicit_starting_xi_unavailable"];
    ...
    const r16Participated = Number.isFinite(r16Points) && r16Points > 0;
    const r32Participated = Number.isFinite(r32Points) && r32Points > 0;
    let evidenceStrength = "prior_only";
    let roleInterpretation = "QF role starts from the R16 role model; explicit R16 starting XI/minutes evidence is unavailable.";
    ...
    } else if (r16Participated && r32Participated && strongPrior) {
      startProb = Math.max(startProb, 0.86);
      expectedMinutes = Math.max(expectedMinutes, 72);
      evidenceStrength = "r16_r32_participation_points_plus_strong_prior";
      roleInterpretation = "R16 and R32 fantasy points confirm repeated participation, and prior role is strong; QF start probability is preserved high without claiming source-backed XI.";
      dataQualityFlags.push("r16_r32_continuity_preserved");
      dataQualityFlags.push("r16_participation_points_weighted");
    } else if (r16Participated && strongPrior) {
      startProb = Math.max(startProb, 0.82);
      expectedMinutes = Math.max(expectedMinutes, 66);
      evidenceStrength = "r16_participation_points_plus_strong_prior";
      roleInterpretation = "R16 fantasy points confirm participation with a strong prior role; QF start probability is lifted, but explicit lineup evidence is unavailable.";
      dataQualityFlags.push("r16_participation_points_weighted");
    } else if (r16Participated) {
      startProb = Math.max(startProb, r16Points >= 4 ? 0.72 : 0.62);
      expectedMinutes = Math.max(expectedMinutes, r16Points >= 4 ? 58 : 44);
      evidenceStrength = "r16_participation_points";
      roleInterpretation = "R16 fantasy points confirm participation; QF role is upgraded from prior-only but remains below explicit starter confidence.";
      dataQualityFlags.push("r16_participation_points_weighted");
    } else if (strongPrior && r32Participated) {
      startProb = Math.min(Math.max(startProb * 0.76, 0.5), 0.68);
      expectedMinutes = Math.min(Math.max(expectedMinutes * 0.76, 42), 58);
      evidenceStrength = "r16_no_points_after_r32_participation";
      roleInterpretation = "Strong R32/prior role lost R16 participation evidence; QF role is downgraded as a role warning.";
      dataQualityFlags.push("r16_no_points_downgraded");
    } else {
      startProb = Math.min(startProb * 0.72, 0.42);
      expectedMinutes = Math.min(expectedMinutes * 0.72, 36);
      evidenceStrength = "r16_no_points_or_unclear_participation";
    ...
    return {
      ...
      roleConfidence: evidenceStrength.includes("strong_prior") || evidenceStrength.includes("continuity") ? "high" : evidenceStrength.includes("r16_participation") ? "medium" : "low",
      role_confidence: evidenceStrength.includes("strong_prior") || evidenceStrength.includes("continuity") ? "high" : evidenceStrength.includes("r16_participation") ? "medium" : "low",
      evidenceStrength,
      role_interpretation: roleInterpretation,
      r16_round_points: r16Points,
      r32_round_points: r32Points,
      r16_participated: r16Participated,
      r32_participated: r32Participated,
  ...
  for (const row of rows) {
    ...
    if (row.public_pool === "main" && Number(row.start_probability) > 0) {
      const penalty = volatility.volatility_score * (row.r16_participated ? 0.08 : 0.16);
  ...
  const summary = {
    ...
    r16_participation_points_weighted: rows.filter((row) => row.data_quality_flags.includes("r16_participation_points_weighted")).length,
    r16_r32_continuity_preserved: rows.filter((row) => row.data_quality_flags.includes("r16_r32_continuity_preserved")).length,
    r16_no_points_downgraded: rows.filter((row) => row.data_quality_flags.includes("r16_no_points_downgraded")).length,
    unavailable_zeroed: rows.filter((row) => row.data_quality_flags.includes("official_unavailable_zeroed")).length,
    eliminated_team_players_excluded: rows.filter((row) => row.data_quality_flags.includes("team_not_in_qf")).length,
  ...
  return {
    ...
    source_files: ["data/officialFantasyPlayers_v0.json", "data/livePlayerStatus_v1.json", "data/fantasyPoolMatchdayProjections_r16_v1.json", "data/qfFixtureAuthority_v1.json"],
    summary,
    playerRoleRows: rows
  };
}
...
function buildProjections({ authority, role, score, r16Projections }) {
  ...
  const rows = rowsFromJson(r16Projections, ["playerMatchdayProjections"])
    ...
    .map((prior) => {
      ...
      return {
        ...
        projectionReason: "QF final projection uses final QF fixture, R16 participation evidence, prior R16 role, completed R16 fixtures, and path value beyond QF.",
        caution: [
          "Explicit R16 starting XI data is unavailable; R16 fantasy points are weaker participation evidence.",
  ...
  return {
    ...
    safety_labels: ["Quarterfinal fantasy setup", "final QF fixtures only for public main rows", "R16 participation evidence weighted", "role volatility included", "ownership not used as signal", "final squads not source-backed"],
    input_files: ["data/playerRoleModel_qf_v1.json", "data/scorePredictions_fantasyPool_qf_v1.json", "data/fantasyPoolMatchdayProjections_r16_v1.json"],
    model: {
      defaultMatchday: MATCHDAY_ID,
      r16_participation_evidence_weighted: true,
...
function buildRecommendations({ projections }) {
  ...
  return {
    ...
    safety_labels: ["Quarterfinal fantasy setup", "final QF fixtures only in main public picks", "R16 participation evidence weighted", "path beyond QF matters", "ownership not used as signal", "final squads not source-backed"],
    input_files: ["data/fantasyPoolMatchdayProjections_qf_v1.json", "data/qfFixtureAuthority_v1.json"],
    model: {
      defaultMatchday: MATCHDAY_ID,
...
function teamBuilderQa({ projections, recommendations }) {
  ...
  return {
    ...
    balanced_sample: balanced.map((row) => ({ name: row.name, country: row.country, position: row.official_fantasy_position, points: row.risk_adjusted_points, opponent: row.opponent })).slice(0, 15),
    greedy_sample: greedy.map((row) => ({ name: row.name, country: row.country, position: row.official_fantasy_position, points: row.risk_adjusted_points, opponent: row.opponent })).slice(0, 15)
  };
}
...
function qaReports({ authority, role, score, projections, recommendations, teamBuilder }) {
  const participation = role.playerRoleRows.filter((row) => row.data_quality_flags.includes("r16_participation_points_weighted")).slice(0, 25);
  const continuity = role.playerRoleRows.filter((row) => row.data_quality_flags.includes("r16_r32_continuity_preserved")).slice(0, 25);
  const downgraded = role.playerRoleRows.filter((row) => row.data_quality_flags.includes("r16_no_points_downgraded")).slice(0, 25);
  ...
  const roleQa = {
    ...
    generated_at: GENERATED_AT,
    status: role.summary.r16_participation_points_weighted > 0 && role.summary.r16_no_points_downgraded > 0 && role.summary.unavailable_zeroed > 0 ? "pass" : "fail",
    ...
    top_qf_starters_by_confidence: role.playerRoleRows
      ...
      .slice(0, 25),
    r16_participation_preserved: participation,
    r16_r32_continuity_preserved: continuity,
    r16_no_points_downgraded: downgraded,
  ...
  const recommendationQa = {
    ...
    r16_participation_included: participation.filter((row) => recommendations.recommendationCandidates.some((candidate) => candidate.official_fantasy_player_id === row.official_fantasy_player_id)).slice(0, 20),
    r16_no_points_downgraded: downgraded,
    role_volatility_audits: playerAudits,
  ...
  const releaseQa = {
    ...
    remaining_limits: [
      ...
      "Explicit R16 starting XI/minutes are unavailable in the official fantasy feed; R16 points are weaker participation evidence.",
...
function buildPostmortem({ r16Score, live, role }) {
  ...
  const dataset = {
    ...
    r16_player_participation_examples: role.playerRoleRows
      .filter((row) => row.public_pool === "main")
      .sort((a, b) => Number(b.r16_round_points || 0) - Number(a.r16_round_points || 0))
      .slice(0, 40)
      .map((row) => ({
        name: row.name,
        country: row.country,
...
export async function buildQfArtifacts() {
  ...
  const role = buildRoleModel({ authority, officialPlayers, livePlayers, r16Projections });
