...
const PATHS = {
  ...
  scoreV1: "data/scorePredictions_v1.json",
  scoreQaV1: "data/scorePredictionQa_v1.json",
  scoreQaReportV1: "data/scorePredictionQaReport_v1.md",
  playerMatchdayV1: "data/playerMatchdayProjections_v1.json",
  matchdayRecommendationsV1: "data/matchdayRecommendations_v1.json",
  recommendationQaV1: "data/recommendationQa_v1.json",
  recommendationQaReportV1: "data/recommendationQaReport_v1.md",
...
function percentileScores(rows, keyFn, targetKey) {
  ...
  for (const row of rows) {
    ...
    row.strategy_scores_v1[targetKey] = round(normalized(value, range) ?? 0, 2);
  }
}
...
async function buildTeamQuality(peleData) {
  ...
  const oldQuality = await readJson(PATHS.teamQualityV0);
  ...
  const preliminary = teams.map(({ sourceTeam, old, pele }) => {
    ...
    const currentStrengthScore = round(weightedAvailable([
      { value: peleScore, weight: 0.4 },
      { value: old.current_strength_inputs.elo_score, weight: 0.34 },
      { value: old.current_strength_inputs.fifa_points_score, weight: 0.26 }
    ]), 2);
    ...
    const overall = round((currentStrengthScore * 0.7) + (historical * 0.17) + (knockout * 0.08) + (hostBonus * 0.05), 2);

    const historyScores = old.world_cup_history_component_scores || {};
    const attackProxyScore = round(weightedAvailable([
      { value: currentStrengthScore, weight: 0.42 },
      { value: peleOffenseScore, weight: 0.32 },
      { value: historyScores.goals_for_per_match_score, weight: 0.14 },
      { value: old.goals_clean_sheet_inputs_v0?.failed_to_score_inverse_score, weight: 0.12 }
    ]), 2);
    const defenseProxyScore = round(weightedAvailable([
      { value: currentStrengthScore, weight: 0.42 },
      { value: peleDefenseScore, weight: 0.32 },
      { value: old.goals_clean_sheet_inputs_v0?.goals_against_inverse_score, weight: 0.14 },
      { value: historyScores.clean_sheet_rate_score, weight: 0.12 }
    ]), 2);
    ...
    const teamQualityV1 = {
      ...
      formula_version: "team_quality_v1_pele_2026-06-01",
      formula_note: "0.70 current strength, 0.17 historical World Cup performance, 0.08 knockout experience, 0.05 host bonus. Current strength uses normalized PELE rating, World Football Elo, and FIFA ranking points. Missing numeric inputs are left null and excluded from their weighted blend.",
      source_ids: ["peleRatings", "fifaRankings", "worldFootballElo", "fjelstulWorldCup", "openFootballWorldCup", "fifaSchedule"],
      data_status: "calculated_v1_with_pele"
    };
    ...
    return {
      ...
      old,
      pele,
      team_quality_v1: teamQualityV1,
      current_strength_inputs_v1: {
        ...
        current_strength_score_formula: "0.40 normalized PELE + 0.34 normalized World Football Elo + 0.26 normalized FIFA ranking points",
        pele_status: "downloaded_from_silver_bulletin_datawrapper_csv"
      },
      goals_clean_sheet_inputs_v1: {
        ...
        note: "Attack and defense proxy scores now blend current strength with PELE round-robin goals-for/goals-against where available. Missing PELE values are not imputed."
      }
    };
  });
  ...
  for (const team of preliminary) {
    ...
    const opponentAverage = round(opponents.reduce((sum, opponent) => sum + opponent.team_quality_v1.overall_score, 0) / opponents.length, 2);
    const qualityGap = round(team.team_quality_v1.overall_score - opponentAverage, 2);
    const groupTeams = preliminary
      .filter((candidate) => candidate.group === team.group)
      .sort((a, b) => b.team_quality_v1.overall_score - a.team_quality_v1.overall_score);
    ...
    team.goals_clean_sheet_inputs_v1.group_goal_scoring_proxy = round(
      team.goals_clean_sheet_inputs_v1.attack_proxy_score * 0.76 + groupEase * 0.24,
      2
    );
    team.goals_clean_sheet_inputs_v1.clean_sheet_potential_proxy = round(
      team.goals_clean_sheet_inputs_v1.defense_proxy_score * 0.76 + groupEase * 0.24,
      2
    );

    team.group_outlook_v1 = {
    ...
    team.tournament_outlook_v1 = {
      playoff_readiness_proxy_score: round(team.team_quality_v1.overall_score * 0.76 + team.team_quality_v1.knockout_experience_score * 0.24, 2),
      likely_role: team.team_quality_v1.quality_tier,
  ...
  const outputTeams = preliminary
    .map((team) => ({
      ...
      current_strength_inputs: team.current_strength_inputs_v1,
      previous_current_strength_inputs_v0: team.old.current_strength_inputs,
      ...
      goals_clean_sheet_inputs_v1: team.goals_clean_sheet_inputs_v1,
      ...
      group_outlook_v1: team.group_outlook_v1,
      group_outlook_v0: team.old.group_outlook_v0,
      tournament_outlook_v1: team.tournament_outlook_v1,
      tournament_outlook_v0: team.old.tournament_outlook_v0
    }))
    .sort((a, b) => b.team_quality_v1.overall_score - a.team_quality_v1.overall_score);

  const qualityOutput = {
    schema_version: "week6-team-quality-v2",
    generated_at: NOW,
    source_checked: TODAY,
    data_status: "team_quality_v1_calculated_for_48_teams_with_pele",
    team_count: outputTeams.length,
    preserved_previous_model_file: PATHS.teamQualityV0,
    model: {
      model_id: "team_quality_v1",
      formula_version: "team_quality_v1_pele_2026-06-01",
      overall_score_formula: "0.70 current_strength_score + 0.17 historical_performance_score + 0.08 knockout_experience_score + 0.05 host_bonus_score",
      current_strength_score_formula: "0.40 normalized PELE + 0.34 normalized World Football Elo + 0.26 normalized FIFA ranking points",
      attack_proxy_formula: "0.42 current strength + 0.32 normalized PELE offense GF + 0.14 historical goals-for score + 0.12 historical failed-to-score inverse score",
      defense_proxy_formula: "0.42 current strength + 0.32 normalized inverse PELE defense GA + 0.14 historical goals-against inverse score + 0.12 historical clean-sheet score",
  ...
  const updatedTeams = {
    ...
    generated_at: NOW,
    source_checked: TODAY,
    data_status: "real_team_identity_groups_fifa_ranking_elo_pele_and_team_quality_v1_complete",
    sources: {
      ...teamsJson.sources,
      pele_ratings_article: PELE_SOURCES.article,
      pele_ratings_csv: PELE_SOURCES.ratingsCsv,
      pele_tilt_csv: PELE_SOURCES.tiltCsv,
      pele_offense_defense_csv: PELE_SOURCES.offenseDefenseCsv,
      team_quality_v1: "data/teamQuality.json",
    ...
    source_notes: [
      ...
      "team_quality_v1 is calculated from PELE, World Football Elo, FIFA ranking points, Fjelstul historical World Cup data, group context, and host status. The pre-PELE model is preserved in data/teamQuality_v0.json."
    ],
    teams: teamsJson.teams.map((team) => {
      ...
      return {
        ...
        data_quality: {
          ...
          prediction_readiness: "ready_for_v1_team_strength",
          team_quality_v1: "calculated_from_pele_rankings_elo_fifa_and_world_cup_history"
        },
        group_prediction_inputs: {
          ...
          available_for_score_prediction_v1: true,
          missing_prediction_inputs: missingInputs
        },
        team_quality_v1: quality.team_quality_v1,
        team_quality_active_version: "team_quality_v1",
        goals_clean_sheet_inputs_v1: quality.goals_clean_sheet_inputs_v1,
        group_outlook_v1: quality.group_outlook_v1,
        tournament_outlook_v1: quality.tournament_outlook_v1
      };
    })
  };
...
function expectedGoalsComponents(team, opponent, fixture, isListedHome) {
  const qualityGap = team.team_quality_v1.overall_score - opponent.team_quality_v1.overall_score;
  const eloGap = team.current_strength_inputs.team_elo - opponent.current_strength_inputs.team_elo;
  const peleGap = team.current_strength_inputs.pele_rating - opponent.current_strength_inputs.pele_rating;
  const tilt = ((team.current_strength_inputs.pele_tilt || 0) + (opponent.current_strength_inputs.pele_tilt || 0)) / 2;
  const hostBoost = fixture && hostVenueCountry(fixture) === team.team_id ? 0.12 : 0;
  const components = {
    base_world_cup_team_goals: 1.33,
    attack_adjustment: ((team.goals_clean_sheet_inputs_v1.attack_proxy_score - 50) / 50) * 0.55,
    opponent_defense_adjustment: ((50 - opponent.goals_clean_sheet_inputs_v1.defense_proxy_score) / 50) * 0.45,
    quality_gap_adjustment: (qualityGap / 100) * 0.75,
    elo_gap_adjustment: (eloGap / 400) * 0.12,
    pele_gap_adjustment: (peleGap / 400) * 0.18,
    tilt_total_goals_adjustment: tilt / 2,
...
async function buildScorePredictions(teamQuality) {
  ...
  for (const fixture of fixtures.fixtures) {
    ...
    fixtureScorePredictions.push({
      prediction_id: `${fixture.match_id}-score-v1`,
      ...
      model_inputs_v1: {
        home_team_quality_score: home.team_quality_v1.overall_score,
        away_team_quality_score: away.team_quality_v1.overall_score,
        home_attack_proxy_score: home.goals_clean_sheet_inputs_v1.attack_proxy_score,
        away_attack_proxy_score: away.goals_clean_sheet_inputs_v1.attack_proxy_score,
        home_defense_proxy_score: home.goals_clean_sheet_inputs_v1.defense_proxy_score,
        away_defense_proxy_score: away.goals_clean_sheet_inputs_v1.defense_proxy_score,
      ...
      formula_version: "pele_team_quality_poisson_score_prediction_v1",
      ...
      data_quality: {
        prediction_status: "prototype_model_output_v1_with_pele",
    ...
    teamFixturePredictions.push({
      team_fixture_prediction_id: `${fixture.match_id}-${home.team_id}-score-v1`,
      fixture_id: fixture.match_id,
      match_number: fixture.match_number,
      fantasy_matchday_id: fantasyMatchdayByFixture.get(fixture.match_id) || null,
    ...
    teamFixturePredictions.push({
      team_fixture_prediction_id: `${fixture.match_id}-${away.team_id}-score-v1`,
      fixture_id: fixture.match_id,
      match_number: fixture.match_number,
      fantasy_matchday_id: fantasyMatchdayByFixture.get(fixture.match_id) || null,
  ...
  const output = {
    schema_version: "week6-score-predictions-v1",
    generated_at: NOW,
    source_checked: TODAY,
    data_status: "prototype_score_prediction_model_v1_with_pele",
    previous_model_file: "data/scorePredictions_v0.json",
    model: {
      model_name: "PELE, Elo, and team-quality adjusted Poisson score model",
      formula_version: "pele_team_quality_poisson_score_prediction_v1",
      base_world_cup_team_goals: 1.33,
      max_score_grid_goals: 10,
      uses_betting_odds: false,
      plain_language_summary: "Estimate each team's expected goals from PELE-backed attack/defense proxies, team-quality gap, Elo gap, PELE rating gap, Tilt, and host-venue boost. Convert expected goals into scoreline probabilities with a Poisson score grid.",
      ...
      future_upgrade_triggers: {
        v2: "Upgrade after final squads, official fantasy prices, official scoring rules, injuries, and pre-tournament national-team form are imported and a backtest/calibration pass is possible."
      }
    },
    summary,
    model_notes: [
      "PELE ratings are imported from Silver Bulletin Datawrapper CSV downloads and are not inferred.",
      "PELE rating, Tilt, and round-robin offense/defense fields are used as model inputs.",
  ...
  await writeJson(PATHS.scoreV1, output);
  return output;
}
...
function matchupNote(country, opponent, context, view) {
  ...
  if (context.fixture_difficulty_score <= 25) {
    return `${country} has a very favorable fantasy matchup against ${opponent}. Score model v1: ${teamXg} team xG, ${xga} xG against, ${cleanSheet}% clean-sheet probability.`;
  }
  if (context.fixture_difficulty_score <= 40) {
    return `${country} faces an easier-than-average group fixture against ${opponent}. Score model v1: ${teamXg} team xG, ${xga} xG against, ${cleanSheet}% clean-sheet probability.`;
  }
  if (context.fixture_difficulty_score >= 75) {
    return `${country} has a very difficult fantasy matchup against ${opponent}; projections and downside floors are reduced. Score model v1: ${teamXg} team xG, ${xga} xG against, ${cleanSheet}% clean-sheet probability.`;
  }
  if (context.fixture_difficulty_score >= 60) {
    return `${country} has a difficult fantasy matchup against ${opponent}. Score model v1: ${teamXg} team xG, ${xga} xG against, ${cleanSheet}% clean-sheet probability.`;
  }
  return `${country} has a roughly neutral fantasy matchup against ${opponent}. Score model v1: ${teamXg} team xG, ${xga} xG against, ${cleanSheet}% clean-sheet probability.`;
}
...
async function buildMatchdayProjections(scorePredictions, teamQuality) {
  ...
  const rows = previous.playerMatchdayProjections.map((row) => {
    ...
    const newScore = {
      model_status: "score_prediction_v1_applied",
    ...
    const newMatch = matchupMultipliers(view, team.team_quality_v1.overall_score, opponent.team_quality_v1.overall_score, avgDifficultyByTeam.get(row.team_id), row.position);
    ...
    return {
      ...row,
      player_matchday_projection_id: row.player_matchday_projection_id.replace(/-md([123])$/, "-md$1-v1"),
      matchup_context_v1: newMatch,
      score_prediction_context_v1: newScore,
      projections_v1: projections,
      strategy_scores_v1: {},
      source_note: "Prototype matchday projection from playerFinanceMetrics_v0, playerRecommendationInputs_v0 group fixture context, playerMinutesModel_v0, and scorePredictions_v1 PELE match environment.",
      data_quality: {
        ...row.data_quality,
        projection_status: "prototype_fixture_adjusted_v1_with_pele",
        score_prediction_available: true,
        caveats: [
          "Fixture adjustments use PELE-backed team quality, fixture difficulty, and scorePredictions_v1 match-environment proxies, not observed World Cup 2026 fantasy results.",
  ...
  for (const matchdayId of ["md1", "md2", "md3"]) {
    ...
    percentileScores(matchdayRows, (row) => row.projections_v1.risk_adjusted_return_points, "risk_adjusted");
    percentileScores(matchdayRows, (row) => row.projections_v1.value_at_risk_10_points * 0.5 + row.projections_v1.conditional_value_at_risk_20_points * 0.3 + row.projections_v1.minutes_security_score * 0.02, "safe_floor");
    percentileScores(matchdayRows, (row) => row.projections_v1.upside_p90_points, "upside");
    percentileScores(matchdayRows, (row) => row.projections_v1.attacking_potential_proxy * 0.45 + row.projections_v1.expected_return_points * 6, "attack_heavy");
    percentileScores(matchdayRows, (row) => row.projections_v1.clean_sheet_potential_proxy * 0.55 + row.score_prediction_context_v1.defensive_environment_score * 0.35 + row.projections_v1.value_at_risk_10_points, "defensive_heavy");
    percentileScores(matchdayRows, (row) => row.projections_v1.upside_p90_points * 0.48 + row.projections_v1.volatility_points * 0.65 + row.score_prediction_context_v1.upset_risk_probability * 38, "very_risky");
    percentileScores(matchdayRows, (row) => row.projections_v1.expected_return_points * 4 + row.projections_v1.value_at_risk_10_points * 1.2 - row.projections_v1.tail_risk_score * 0.12, "tail_risk_avoidance");
    percentileScores(matchdayRows, (row) => row.projections_v1.captain_score, "captain");
    percentileScores(matchdayRows, (row) => row.projections_v1.minutes_security_score + row.projections_v1.expected_return_points * 2, "minutes_floor");
  }

  const output = {
    schema_version: "week6-player-matchday-projections-v1",
    generated_at: NOW,
    source_checked: TODAY,
    data_status: "prototype_fixture_adjusted_player_projection_model_v1_with_pele",
    previous_model_file: "data/playerMatchdayProjections_v0.json",
    model_notes: [
      "One row per player per group-stage fixture.",
      "Uses existing playerFinanceMetrics_v0 expected return/risk metrics and adjusts them with PELE-backed fixture difficulty plus scorePredictions_v1 match environment.",
  ...
  await writeJson(PATHS.playerMatchdayV1, output);
  return output;
}
...
function topListRow(row, strategy) {
  return {
    ...
    score: row.strategy_scores_v1[strategy],
    expected_return_points: row.projections_v1.expected_return_points,
    risk_adjusted_return_points: row.projections_v1.risk_adjusted_return_points,
    fixture_difficulty_score: row.matchup_context_v1.fixture_difficulty_score,
    fixture_difficulty_band: row.matchup_context_v1.fixture_difficulty_band,
    team_expected_goals: row.score_prediction_context_v1.expected_goals,
    team_expected_goals_against: row.score_prediction_context_v1.expected_goals_against,
    team_clean_sheet_probability: row.score_prediction_context_v1.clean_sheet_probability,
    team_win_probability: row.score_prediction_context_v1.win_probability,
    team_attacking_environment_score: row.score_prediction_context_v1.attacking_environment_score,
    team_defensive_environment_score: row.score_prediction_context_v1.defensive_environment_score,
    team_captain_environment_score: row.score_prediction_context_v1.captain_environment_score,
    match_goal_environment: row.score_prediction_context_v1.goal_environment,
    match_upset_risk_probability: row.score_prediction_context_v1.upset_risk_probability,
    match_upset_risk_band: row.score_prediction_context_v1.upset_risk_band,
    value_at_risk_10_points: row.projections_v1.value_at_risk_10_points,
    conditional_value_at_risk_20_points: row.projections_v1.conditional_value_at_risk_20_points,
    tail_risk_score: row.projections_v1.tail_risk_score,
...
function aggregateGroupRows(rows) {
  ...
  return [...byPlayer.values()].map((playerRows) => {
    ...
    const bestFixture = [...playerRows].sort((a, b) => b.projections_v1.expected_return_points - a.projections_v1.expected_return_points)[0];
    ...
    row.fixture_id = null;
    row.date = null;
    row.projections_v1.expected_return_points = avg((item) => item.projections_v1.expected_return_points);
    row.projections_v1.risk_adjusted_return_points = avg((item) => item.projections_v1.risk_adjusted_return_points);
    row.projections_v1.value_at_risk_10_points = avg((item) => item.projections_v1.value_at_risk_10_points);
    row.projections_v1.conditional_value_at_risk_20_points = avg((item) => item.projections_v1.conditional_value_at_risk_20_points);
    row.projections_v1.upside_p90_points = avg((item) => item.projections_v1.upside_p90_points);
    row.projections_v1.volatility_points = avg((item) => item.projections_v1.volatility_points);
    row.projections_v1.tail_risk_score = avg((item) => item.projections_v1.tail_risk_score);
    row.projections_v1.composite_risk_score = avg((item) => item.projections_v1.composite_risk_score);
    row.score_prediction_context_v1.expected_goals = avg((item) => item.score_prediction_context_v1.expected_goals);
    row.score_prediction_context_v1.clean_sheet_probability = avg((item) => item.score_prediction_context_v1.clean_sheet_probability);
    row.score_prediction_context_v1.upset_risk_probability = avg((item) => item.score_prediction_context_v1.upset_risk_probability);
    row.matchup_context_v1.fixture_difficulty_score = avg((item) => item.matchup_context_v1.fixture_difficulty_score);
    row.strategy_scores_v1 = {};
    for (const strategy of STRATEGIES) {
      row.strategy_scores_v1[strategy] = avg((item) => item.strategy_scores_v1[strategy] || 0);
    }
    return row;
  });
}
...
async function buildMatchdayRecommendations(matchdayProjectionData) {
  ...
  const matchdayRecommendations = matchdayRows.map((matchday) => {
    ...
    for (const strategy of STRATEGIES) {
      top_lists[strategy] = [...matchday.rows]
        .sort((a, b) => (b.strategy_scores_v1[strategy] || 0) - (a.strategy_scores_v1[strategy] || 0))
        .slice(0, 25)
  ...
  const output = {
    schema_version: "week6-matchday-recommendations-v1",
    generated_at: NOW,
    source_checked: TODAY,
    data_status: "prototype_matchday_recommendation_shortlists_v1_with_pele",
    input_file: "playerMatchdayProjections_v1.json",
    previous_model_file: "data/matchdayRecommendations_v0.json",
    summary: {
  ...
  await writeJson(PATHS.matchdayRecommendationsV1, output);
  return output;
}
...
function browserProjectionRow(row) {
  return {
    ...
    fixture_difficulty_score: row.matchup_context_v1.fixture_difficulty_score,
    fixture_difficulty_band: row.matchup_context_v1.fixture_difficulty_band,
    difficulty_delta_vs_group_average: row.matchup_context_v1.difficulty_delta_vs_group_average,
    attack_multiplier: row.matchup_context_v1.attack_multiplier,
    defense_multiplier: row.matchup_context_v1.defense_multiplier,
    matchup_note: row.matchup_context_v1.matchup_note,
    team_expected_goals: row.score_prediction_context_v1.expected_goals,
    team_expected_goals_against: row.score_prediction_context_v1.expected_goals_against,
    team_win_probability: row.score_prediction_context_v1.win_probability,
    team_draw_probability: row.score_prediction_context_v1.draw_probability,
    team_loss_probability: row.score_prediction_context_v1.loss_probability,
    team_clean_sheet_probability: row.score_prediction_context_v1.clean_sheet_probability,
    team_attacking_environment_score: row.score_prediction_context_v1.attacking_environment_score,
    team_defensive_environment_score: row.score_prediction_context_v1.defensive_environment_score,
    team_captain_environment_score: row.score_prediction_context_v1.captain_environment_score,
    ...
    match_upset_risk_probability: row.score_prediction_context_v1.upset_risk_probability,
    match_upset_risk_band: row.score_prediction_context_v1.upset_risk_band,
    finance_expected_return_points: row.projections_v1.expected_return_points,
    finance_risk_adjusted_return_points: row.projections_v1.risk_adjusted_return_points,
    finance_var10_points: row.projections_v1.value_at_risk_10_points,
    finance_cvar20_points: row.projections_v1.conditional_value_at_risk_20_points,
    finance_volatility_points: row.projections_v1.volatility_points,
    finance_downside_deviation_points: row.projections_v1.downside_deviation_points,
    finance_upside_p90_points: row.projections_v1.upside_p90_points,
    finance_bad_week_probability: row.projections_v1.bad_week_probability,
    finance_composite_risk_score: row.projections_v1.composite_risk_score,
    finance_tail_risk_score: row.projections_v1.tail_risk_score,
    finance_minutes_security_score: row.projections_v1.minutes_security_score,
    finance_captain_score: row.projections_v1.captain_score,
    finance_clean_sheet_potential_proxy: row.projections_v1.clean_sheet_potential_proxy,
    finance_attacking_potential_proxy: row.projections_v1.attacking_potential_proxy,
    finance_strategy_risk_adjusted: row.strategy_scores_v1.risk_adjusted,
    finance_strategy_safe_floor: row.strategy_scores_v1.safe_floor,
    finance_strategy_upside: row.strategy_scores_v1.upside,
    finance_strategy_attack_heavy: row.strategy_scores_v1.attack_heavy,
    finance_strategy_defensive_heavy: row.strategy_scores_v1.defensive_heavy,
    finance_strategy_very_risky: row.strategy_scores_v1.very_risky,
    finance_strategy_minutes_floor: row.strategy_scores_v1.minutes_floor,
    finance_strategy_tail_risk_avoidance: row.strategy_scores_v1.tail_risk_avoidance,
    finance_strategy_captain: row.strategy_scores_v1.captain,
...
async function writeBrowserFiles(scorePredictions, scoreQa, matchdayProjectionData, matchdayRecommendations) {
  ...
  const scoreSummary = {
    schema_version: "score-predictions-browser-data-v1",
    generated_at: NOW,
    ...
    source_files: ["data/scorePredictions_v1.json"],
  ...
  const matchdaySummary = {
    schema_version: "matchday-browser-data-v1",
    generated_at: NOW,
...
async function buildScoreQa(scorePredictions, teamQuality, matchdayProjectionData) {
  ...
  pushCheck("player_matchday_score_context", "Player matchday score context", matchdayProjectionData.playerMatchdayProjections.every((row) => row.score_prediction_context_v1?.model_status === "score_prediction_v1_applied"), `${matchdayProjectionData.playerMatchdayProjections.length}/${matchdayProjectionData.playerMatchdayProjections.length} player-matchday rows have score-prediction context.`);
  pushCheck("player_multiplier_bounds", "Player environment multiplier bounds", matchdayProjectionData.playerMatchdayProjections.every((row) => {
    const context = row.score_prediction_context_v1;
  ...
  const qa = {
    schema_version: "week6-score-prediction-qa-v1",
    generated_at: NOW,
    source_checked: TODAY,
    data_status: "score_prediction_hardening_v1_pele",
    overall_status: checksFailed ? "fail" : "pass",
    source_files: ["data/scorePredictions_v1.json", "data/fixtures.json", "data/matchdays.json", "data/teams.json", "data/teamQuality.json", "data/peleRatings_v1.json", "data/playerMatchdayProjections_v1.json"],
    summary: {
      ...
      player_matchday_rows_with_score_context: matchdayProjectionData.playerMatchdayProjections.filter((row) => row.score_prediction_context_v1).length,
...
async function buildRecommendationQa(matchdayProjectionData) {
  ...
  for (const matchday of auditMatchdays) {
    for (const [style, strategy] of Object.entries(styleMap)) {
      const top = [...matchday.rows].sort((a, b) => (b.strategy_scores_v1[strategy] || 0) - (a.strategy_scores_v1[strategy] || 0)).slice(0, 25);
      ...
      for (const row of top) {
        ...
        if ((row.projections_v1?.composite_risk_score || 0) >= 70) warnings.high_composite_risk = (warnings.high_composite_risk || 0) + 1;
        if ((row.matchup_context_v1?.fixture_difficulty_score || 0) >= 70) warnings.hard_fixture = (warnings.hard_fixture || 0) + 1;
      }
      matchday_fixture_audit.push({
        ...
        average_team_expected_goals: round(top.reduce((sum, row) => sum + row.score_prediction_context_v1.expected_goals, 0) / Math.max(1, top.length), 2),
        average_clean_sheet_probability: round(top.reduce((sum, row) => sum + row.score_prediction_context_v1.clean_sheet_probability, 0) / Math.max(1, top.length), 3),
        average_upset_risk_probability: round(top.reduce((sum, row) => sum + row.score_prediction_context_v1.upset_risk_probability, 0) / Math.max(1, top.length), 3),
        warnings
      });
    }
  }
  ...
  const qa = {
    schema_version: "week6-recommendation-qa-v1",
    generated_at: NOW,
    source_checked: TODAY,
    scope: {
      ...
      note: "Audits the browser-ready matchday recommendation data after PELE-backed score prediction v1."
    },
    source_files: [
    ...
    global_summary: {
      ...
      high_risk_projection_count: browserRows.filter((row) => row.projections_v1.composite_risk_score >= 70).length,
      hard_fixture_projection_count: browserRows.filter((row) => row.matchup_context_v1.fixture_difficulty_score >= 70).length,
  ...
  await writeJson(PATHS.recommendationQaV1, qa);
