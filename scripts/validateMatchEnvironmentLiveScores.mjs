  ...
  return [
    ...
    mdTable(
      ...
      [
        ...
        ["QF default", qa.summary.qf_default ? "yes" : "no"],
        ["QF/R16/R32/MD views accessible", qa.summary.active_matchdays_accessible ? "yes" : "no"]
      ]
    ),
    "",
...
const [html, scriptText, r32ScorePredictionData, r16ScorePredictionData, qfScorePredictionData, liveMatchdayData, liveFixtureQa] = await Promise.all([
...
const scoreRows = [...qfScoreRows, ...r16ScoreRows, ...r32ScoreRows];
...
const qfDefault = /defaultPublicMatchdayId\s*=\s*"qf"/.test(scriptText) &&
  /let activeEnvironmentMatchdayId\s*=\s*defaultActiveMatchdayId/.test(scriptText);
const activeMatchdaysAccessible = /matchday_id:\s*"qf"/.test(scriptText) &&
...
if (!qfDefault) errors.push("Match Environment does not default to final QF.");
if (!activeMatchdaysAccessible) errors.push("QF, R16, R32, MD1, MD2, and MD3 are not all available in matchday options.");
